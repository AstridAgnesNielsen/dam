...
# I probably don't want to include them all in a word cloud. Let's filter to only include the top 100 most frequent?
...
got_afinn2_n <- got_afinn2 %>% 
  ...
  top_n(50) %>% # I have too many words, so we'll just select the top 50 most frequent words so the plot is readable.
...
I can use the NRC lexicon to start "binning" text by the feelings they're typically associated with. As above, we'll use inner_join() to combine the GOT non-stopword text with the nrc lexicon:
```{r bind-bing}
got_nrc <- got_stop %>% 
  inner_join(get_sentiments("nrc"))
```

Wait, won't that exclude some of the words in our text? YES! I should check which are excluded using `anti_join()`:
